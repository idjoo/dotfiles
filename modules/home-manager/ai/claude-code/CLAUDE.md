...
| **AskUserQuestion** | Use for ALL user interactions (choices, clarifications, interviews)                                 |
...
- Ask questions ONE at a time, prefer multiple choice via `AskUserQuestion`
...
- Fresh subagent per task via `Task` tool — no context pollution
...
3. Dispatch in parallel via `Task` tool in a single message
...
ALWAYS use `AskUserQuestion` when interacting with the user. If ANY of these apply, use it:
...
- Use `multiSelect: true` when user can select more than one
- Keep labels concise (1-5 words)
- Batch related questions in one call

```
CORRECT:   AskUserQuestion(header: "Auth Method", options: ["JWT (Recommended)", "Session", "OAuth"])
...
Delegate specialized tasks to appropriate subagents via the `Task` tool.

<delegation-guide>

| Agent Type      | Task                    | Description                                    |
| --------------- | ----------------------- | ---------------------------------------------- |
| Explore         | Codebase exploration    | Find files, patterns, implementations          |
| Plan            | Architecture planning   | Design implementation strategy                 |
| general-purpose | Complex research        | Multi-step investigation, parallel work        |

</delegation-guide>

<delegation-rules>

- Parallel agents: Launch multiple agents in ONE message for broad searches
- Be specific: Provide focused prompts with clear success criteria
- Background when possible: Use `run_in_background: true` for non-blocking work
...
| AskUserQuestion| Asking choices/options in plain text instead of using the tool         |
