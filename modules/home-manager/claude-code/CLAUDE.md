...
| **Bash**      | Chain commands in ONE call; use `run_in_background: true`    |
...
### Bash: Always Background

**ALWAYS use `run_in_background: true`** for Bash commands:

```
✓ Bash(command, run_in_background: true) → TaskOutput(task_id)
✗ Bash(command)  ← Blocks execution
```

Chain shell commands in ONE Bash call:

```
✓ Bash("npm test && npm run lint && npm run build", run_in_background: true)
...
Use `TaskOutput` to get results from background commands:

```
1. Bash(command, run_in_background: true) → returns task_id
2. TaskOutput(task_id, block: true)       → waits for completion
3. TaskOutput(task_id, block: false)      → non-blocking status check
```
